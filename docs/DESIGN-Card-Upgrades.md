...
DEF is a percentage damage reducer and receives **+3% per chosen level**. Engine and UI
retain one decimal place so that increase remains visible on the roster's smaller DEF
...
Each level grants **+5% ATK, +7% HP, or +3% DEF** for that level's chosen booster
