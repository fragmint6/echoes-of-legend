...
['data/_schema.js', 'data/roles.js',
 'data/camelot.js', 'data/olympus.js', 'data/sherwood.js',
 'data/grimmwood.js', 'data/yamato.js', 'data/huaxia.js',
 'js/engine.js', 'js/ai.js'].forEach(f => {
...
const HERO = {};
POOL.forEach(e => { HERO[e.card.id] = e.card; });
...
function mkHeroStats() {
  return {
    apps: 0, wins: 0, draws: 0, p1Apps: 0, p1Wins: 0,
    dmg: 0, dmgTaken: 0, heals: 0, shields: 0, absorbCredit: 0,
    prevented: 0, kills: 0, deaths: 0,
    energy: 0, basics: 0, sigs: 0, sigEnergy: 0,
    crits: 0, hits: 0, burnDmg: 0,
    exposedBonusDealt: 0, exposedBonusEnabled: 0,
    dmgVsTank: 0, dmgVsBackline: 0,
    tauntTurnsApplied: 0, redirects: 0,
    buffUpN: 0, debuffUpN: 0, upSamples: 0,
    overkill: 0, targeted: 0, teamTargeted: 0,
    focusN: 0, focusD: 0,
    dmgBeforeDeath: 0, deathGames: 0, dmgAfterFirstKill: 0,
    mvp: 0, aliveAtEnd: 0,
    lastSurvivorGames: 0, lastSurvivorWins: 0,
    firstKills: 0, firstKillWins: 0,
    concededFK: 0, concededFKWins: 0,
    firstKillRoundSum: 0, killGames: 0,
    kpSum: 0, roundsSum: 0, dmgPerRoundSum: 0
  };
}
function mkAbilityStats() {
  return { casts: 0, value: 0, kills: 0, targetsHit: 0,
           dmg: 0, heal: 0, shield: 0, buffs: 0, debuffs: 0,
           heroName: '', kind: '' };
}
...
const A = {
  ...
  rounds: [], actionsPerGame: [], roundsPerGame: [],
  sigRoundsFirst: [], firstKillRounds: [], secondKillRounds: [],
  winnerAliveLeft: [], winnerHpLeft: [],
  fkDecisiveGames: 0, fkConverted: 0, gamesWithKill: 0,
  sigCasts: 0, basicCasts: 0,
  heroes: {}, roles: {}, abilities: {}, basics: {},
  statuses: {},
  burn: { ticks: 0, tickDmg: 0, kills: 0 },
  exposed: { dmgWhile: 0, killsWhile: 0 },
  mark: { triggers: 0, triggerDmg: 0 },
  pairs: {}, rolePairs: {}, matchups: {}, comps: {},
  pos: {
...
ROLES.forEach(r => { A.roles[r] = mkHeroStats(); });
['burn', 'exposed', 'marked', 'silence', 'taunt', 'healMod', 'untargetable']
  .forEach(s => { A.statuses[s] = mkStatusStats(); });
...
function runGame(seed) {
  ...
  const g0 = () => ({
    win: 0, draw: 0, p1: 0, dmg: 0, dmgTaken: 0, heals: 0, shields: 0,
    absorbCredit: 0, prevented: 0, kills: 0, deaths: 0, died: false,
    deathRound: -1, energy: 0, basics: 0, sigs: 0, sigEnergy: 0,
    crits: 0, hits: 0, burnDmg: 0, exposedBonusDealt: 0,
    exposedBonusEnabled: 0, dmgVsTank: 0, dmgVsBackline: 0,
    tauntTurnsApplied: 0, redirects: 0, buffUpN: 0, debuffUpN: 0,
    upSamples: 0, overkill: 0, targeted: 0, teamTargeted: 0,
    focusN: 0, focusD: 0, dmgSoFar: 0, dmgBeforeDeath: 0,
    dmgAfterFirstKill: 0, mvp: 0, aliveAtEnd: 0, lastSurvivor: 0,
    gotFirstKill: 0, concededFK: 0, firstKillRoundSum: 0, kp: 0, rounds: 0
  });
  B.units.forEach(u => {
    const passive = u.card.ability.type === 'Passive';
    U[u.uid] = { id: u.card.id, name: u.name, role: u.role, side: u.side,
                 slot: u.slot,
  ...
  let actions = 0;
  let firstDeathRound = 0, secondDeathRound = 0, firstKillerId = null, firstSigRound = 0;
  const lastDmgOn = {};
  const lastExposer = {};
  ...
  function watchFlags() {
    ...
    B.units.forEach(u => {
      const f = {};
      ['burn', 'exposed', 'marked', 'silence', 'taunt', 'healMod', 'untargetable']
        .forEach(s => { f[s] = (u.flags[s] || 0) > 0 ? 1 : 0; });
      snap[u.uid] = f;
  ...
  EOL.onBattleEvent = function (BB, ev) {
    ...
    switch (ev.t) {
      case 'dmg': {
        const src = U[ev.src], tgt = U[ev.tgt];
        if (!src || !tgt) break;
        const gs = G[src.id], gtt = G[tgt.id];
        gs.dmg += ev.amount; gs.dmgSoFar += ev.amount;
        if (firstDeathRound) gs.dmgAfterFirstKill += ev.amount;
        gtt.dmgTaken += ev.amount;
        gs.hits++; if (ev.crit) gs.crits++;
        if (ev.exposedBonus) {
        ...
        gtt.targeted++;
        B.units.forEach(x => { if (x.side === tgt.side) G[x.card.id].teamTargeted++; });
        if (ev.absorbed) {
        ...
        if (ev.prevents) ev.prevents.forEach(p => {
          if (U[p.owner]) {
        ...
        if (S && ev.src === currentAction.actorUid) {
          S.dmg += ev.amount; S.value += ev.amount; S.targets[ev.tgt] = 1;
      ...
      case 'burnTick': {
        ...
        A.burn.ticks++; A.burn.tickDmg += ev.amount;
      ...
      case 'heal': {
        ...
        if (S && ev.src === currentAction.actorUid) {
          S.heal += ev.amount; S.value += ev.amount; S.targets[ev.tgt] = 1;
        }
        break;
      }
      case 'shield': {
        ...
        if (S && ev.src === currentAction.actorUid) {
          S.shield += ev.amount; S.value += ev.amount;
          S.targets[ev.tgt] = 1; S.buffs++;
        }
        break;
      }
      case 'stat': {
        const src = U[ev.src], tgt = U[ev.tgt];
        if (src && tgt) routePassiveValue(ev.src, 'value',
          Math.abs(ev.amt) * (ev.stat === 'def' ? 30 : 15));
        if (S && ev.src === currentAction.actorUid) {
          S.targets[ev.tgt] = 1;
          if (ev.amt >= 0) S.buffs++; else S.debuffs++;
          // utility value, priced like ai.js scoreEffects
          const u = BB.units.find(x => x.uid === ev.tgt);
          if (u) {
            const base = ev.stat === 'atk' ? E.atkOf(u) * 1.1
                       : ev.stat === 'def' ? u.maxHp * 0.30
                       : E.atkOf(u) * 0.9;
      ...
      case 'statusApply': {
        ...
        const tgtU = U[ev.tgt];
        if (st) {
          if (ev.refreshed) st.refreshed++; else st.applied++;
          ...
          if (S && ev.src === currentAction.actorUid) {
            S.targets[ev.tgt] = 1;
            if (ev.status === 'taunt' || ev.status === 'untargetable' || ev.status === 'counterStrike') S.buffs++;
            else S.debuffs++;
            // utility value, priced like ai.js scoreEffects
            ...
            else if (ev.status === 'exposed' && tu && !ev.refreshed) S.value += 180 + tu.baseDef * 14;
      ...
      case 'death': {
        ...
        if (rr) {
          const g = G[rr.id];
          g.deaths++; g.died = true; g.deathRound = ev.round;
          ...
          ['burn', 'exposed', 'marked', 'silence', 'taunt', 'healMod', 'untargetable']
            .forEach(s => closeStatus(ev.uid, s, 'died', ev.round));
        }
        watchFlags();
        break;
      }
      case 'proc': {
        ...
        if (o && o.passiveKey) {
          ...
          rr.casts++; rr.heroName = o.name; rr.kind = 'Passive';
        }
        break;
      }
  ...
  while (!B.over && B.round <= ROUND_CAP && steps++ < 5000) {
    ...
    const unit = act.unit, ability = act.ability;
    ...
    currentAction = {
      actorUid: unit.uid,
      agg: { dmg: 0, heal: 0, shield: 0, value: 0, kills: 0,
             buffs: 0, debuffs: 0, targets: {} }
    };
    const res = E.useAbility(B, unit, ability, act.chosen, act.choose);
    const bucket = currentAction; currentAction = null;
    if (!res.ok) { B.acted[side][unit.uid] = true; continue; }
    ...
    if (ability.basic) {
      A.basicCasts++; g.basics++;
      const br = A.basics[ability.name] = A.basics[ability.name] || mkAbilityStats();
      foldCast(br, bucket);
    } else {
      A.sigCasts++; g.sigs++; g.sigEnergy += energyBefore;
      if (!firstSigRound) firstSigRound = B.round;
      const rr = abilAgg(unit.card.id + '|' + ability.name);
      rr.heroName = unit.name; rr.kind = 'Signature (' + cost + ' EN)';
      foldCast(rr, bucket);
    }

    const spec = ability.spec || {}, tSpec = spec.target || {};
  ...
  const gp = { winner: winnerSide === 'player' ? 'P1' : winnerSide === 'enemy' ? 'P2' : null,
               draw: !winnerSide, rounds: B.round, actions };

  const teamKills = { player: 0, enemy: 0 };
  B.units.forEach(u => { if (!u.alive) teamKills[E.opposite(u.side)]++; });

  const fkUnit = firstKillerId
    ? B.units.filter(u => u.card.id === firstKillerId)[0] : null;
  const fkWon = !!(fkUnit && gp.winner &&
    gp.winner === (fkUnit.side === 'player' ? 'P1' : 'P2'));
  ...
  B.units.forEach(u => {
    ...
    g.mvp = g.dmg + 0.8 * (g.heals + g.shields + g.absorbCredit + g.prevented)
          + 400 * g.kills;
  });

  EOL.onBattleEvent = null;
  return { B, U, G, gp, firstDeathRound, secondDeathRound, firstSigRound,
           firstKillerId, fkWon };
}
...
function classify(ability) {
  ...
  (function walk(list) {
    (list || []).forEach(e => {
      flat.push(e.k);
      if (e.k === 'branch') { walk(e.then); walk(e.other); }
      if (e.k === 'coinFlip') { walk(e.heads && e.heads.effects); walk(e.tails && e.tails.effects); }
...
function mergeHero(id, g) {
  const h = heroAgg(id), r = A.roles[HERO[id].role];
  [h, r].forEach(t => {
    t.apps++; t.wins += g.win; t.draws += g.draw;
    t.p1Apps += g.p1; t.p1Wins += g.p1 ? g.win : 0;
    t.dmg += g.dmg; t.dmgTaken += g.dmgTaken; t.heals += g.heals;
    t.shields += g.shields; t.absorbCredit += g.absorbCredit;
    t.prevented += g.prevented; t.kills += g.kills; t.deaths += g.deaths;
    t.energy += g.energy; t.basics += g.basics; t.sigs += g.sigs;
    t.sigEnergy += g.sigEnergy; t.crits += g.crits; t.hits += g.hits;
    t.burnDmg += g.burnDmg;
    t.exposedBonusDealt += g.exposedBonusDealt;
    t.exposedBonusEnabled += g.exposedBonusEnabled;
    t.dmgVsTank += g.dmgVsTank; t.dmgVsBackline += g.dmgVsBackline;
    t.tauntTurnsApplied += g.tauntTurnsApplied; t.redirects += g.redirects;
    t.buffUpN += g.buffUpN; t.debuffUpN += g.debuffUpN; t.upSamples += g.upSamples;
    t.overkill += g.overkill; t.targeted += g.targeted;
    t.teamTargeted += g.teamTargeted;
    t.focusN += g.focusN; t.focusD += g.focusD;
    t.dmgBeforeDeath += g.dmgBeforeDeath; t.deathGames += g.died ? 1 : 0;
    t.dmgAfterFirstKill += g.dmgAfterFirstKill;
    t.mvp += g.mvp; t.aliveAtEnd += g.aliveAtEnd;
    ...
    t.kpSum += g.kp; t.roundsSum += g.rounds;
...
function foldGame(gd) {
  ...
  if (gd.firstDeathRound) {
    ...
    if (gp.winner) { A.fkDecisiveGames++; if (gd.fkWon) A.fkConverted++; }
  ...
  B.units.forEach(u => {
    ...
    P.apps++; if (u.alive) P.aliveEnd++;
  });

  /* pairs, role pairs, comps */
  [['player', 'P1'], ['enemy', 'P2']].forEach(([side, label]) => {
  ...
  p1.forEach(a => p2.forEach(b => {
    ...
    m.games++; if (gp.winner === 'P1') m.wins++;
  }));
}
...
for (let i = 0; i < N_GAMES; i++) {
  foldGame(runGame(SEED + i));
  if ((i + 1) % 25 === 0) {
    const dt = (Date.now() - t0) / 1000;
    console.log(`  ${i + 1}/${N_GAMES}  ${dt.toFixed(0)}s  (${(1000 * dt / (i + 1)).toFixed(0)} ms/game)`);
  }
}
