...
(async () => {
  ...
  const cards = await p.evaluate(() => {
    ...
    const P = mk(['camelot-mordred', 'grimmwood-rapunzel', 'camelot-merlin', 'camelot-morgan-le-fay', 'hemithea-hercules', 'huaxia-guan-yu']);
  ...
  await p.evaluate(() => {
    ...
    const cardOf = {};
    EOL.factions.forEach((fa) => fa.cards.forEach((c) => (cardOf[c.id] = { card: c, faction: fa })));
    const P = ['hemithea-hercules', 'huaxia-guan-yu', 'camelot-mordred', 'camelot-guinevere', 'grimmwood-rapunzel', 'olympus-zeus'].map((id) => cardOf[id]);
