...
(function () {
  ...
  function createBattle(playerCards, enemyCards, opts) {
    ...
    /* CARD UPGRADES (docs/DESIGN-Card-Upgrades.md).
       ...
       Only the chosen stat moves: +5% ATK, +7% HP, or +3% DEF
       per chosen level. Skill power is a separate multiplier applied at the effect
    ...
    function applyUpgrades(side, table) {
      ...
      B.units.forEach(function (u) {
        ...
        if (n.def) {
          /* DEF scales relatively like the other raw stats. One decimal
             keeps a +3% choice visible on the roster's smaller values. */
          u.baseDef = Math.round(u.baseDef * (1 + 0.03 * n.def) * 10) / 10;
        }
      });
    }
